  ...
  if (!history || history.length < 2) {
    return (
      <div style={{ height: 80, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-3)" }}>No chart data</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div style={{
        ...
        color: up ? "var(--yes-color)" : "var(--danger)",
      }}>
        {up ? "+" : ""}{pct}%
      </div>
    </div>
...
export function TradingView() {
  ...
  async function submit() {
    ...
    try {
      ...
      setSuccess(`Position opened. Order: ${json.orderId}`);
      setCollateral("");
      setLeverage(1);
      setSide(null);
      refetchPositions();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
          <div style={{
            ...
            border: `1px solid ${side === "YES" ? "var(--yes-color)" : "var(--border)"}`,
            borderRadius: 12,
            padding: "14px 16px",
          ...
          <div style={{
            background: "var(--surface-2)",
            border: `1px solid ${side === "NO" ? "var(--no-color)" : "var(--border)"}`,
            borderRadius: 12,
            padding: "14px 16px",
            transition: "border-color 150ms",
          }}>
            ...
            <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-3)", marginTop: 4 }}>
              Regime survives until July
            </div>
            <MiniChart
              history={(market?.priceHistory ?? []).map((h) => ({ t: h.t, p: 1 - h.p }))}
              color="var(--no-color)"
            />
          </div>
        </div>
      ...
      <div className="card" style={{ padding: 20 }}>
        <div className="card-header">
          <div>
            <div className="metric-label" style={{ marginBottom: 2 }}>Open Leveraged Position</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
          <button
            className={`side-btn side-btn-yes ${side === "YES" ? "active" : ""}`}
            onClick={() => setSide("YES")}
          >
            LONG YES · {yesPrice.toFixed(3)}
          </button>
          <button
            className={`side-btn side-btn-no ${side === "NO" ? "active" : ""}`}
            onClick={() => setSide("NO")}
          >
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            ...
            <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-3)" }}>
              Wallet: <span style={{ color: insufficientBalance ? "var(--danger)" : "var(--text-2)" }}>
                ${walletBalanceNum.toFixed(2)} USDC.e
              </span>
            </span>
          </div>
          <div style={{ position: "relative" }}>
          ...
          {insufficientBalance && (
            <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--danger)", marginTop: 6 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 6 }}>
            {[1, 2, 3, 4, 5].map((x) => (
              <button
                key={x}
                className={`lev-btn ${leverage === x ? "active" : ""}`}
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ background: "var(--surface-2)", borderRadius: 10, padding: "12px 14px" }}>
            ...
            {[
              ...
              { label: "Borrow APR (util-based)", value: `${(preview.fees.borrowApr * 100).toFixed(1)}%` },
            ].map(({ label, value }) => (
              <div key={label} className="summary-row">
                <span className="summary-label">{label}</span>
        ...
        {insufficientLiquidity && (
          <div className="alert-error" style={{ marginBottom: 12 }}>
            ✕ Vault has insufficient liquidity for ${preview.borrowed.toFixed(2)} borrow. Available: ${snapshot?.available.toFixed(2) ?? "0"}.
          </div>
        )}
      ...
      {openPositions.length > 0 && (
        <div className="card" style={{ padding: 20 }}>
          <div className="card-header">
            ...
            <div className="pill pill-live">{openPositions.length} Active</div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {openPositions.map((p) => (
              <div key={p.id} style={{
                ...
                <div style={{ marginTop: 8, fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-3)" }}>
                  ID: {p.id}
                </div>
              </div>
            ))}
